...
import { fauthstorage, database,auth } from "../../../config/config.js";
...
export default class messaginScreen extends Component {
  ...
  check = () => {
    ...
    if (params) {
      if (params.userId) {
        ...
        var that = this;
        var UserIdRef= child(ref(database,"users"),params.userId);
        var firstNameRef= child(UserIdRef,"firstname");
        onValue(firstNameRef,function (snapshot) {
        ...
        var avatarRef = child( UserIdRef , "avatar" );
  ...
  fetchMessages = () => {
    ...
    var userChatRef= child(child(child(ref(database, "users"),userId),"userChats"),this.state.friendId);
    onValue(userChatRef,function (snapshot) {
      const exist = snapshot.exists();
      if (exist) {
        var data = snapshot.val();
        onValue(child(ref(database,"chatMessages"),Object.keys(data)[0]),
        function (snapshot) {
  ...
  sendMessage = () => {
    if (
      ...
      var userId = auth.currentUser.uid;
      var friendIdRef= child(child(child(ref(database,"users"),userId),"userChats"),this.state.friendId);
      onValue(friendIdRef,function (snapshot) {
        const exist = snapshot.exists();
        if (exist) {
          data = snapshot.val();
  ...
  componentDidMount = () => {
    ...
    onAuthStateChanged(auth,function (user) {
      if (user) {
        ...
        that.check();
        var userId = auth.currentUser.uid;
        var userIdRef= child(ref(database,"users"),userId);
        var nameRef= child(userIdRef,"name");
        onValue(nameRef,function (snapshot) {
          const exist = snapshot.val() != null;
          if (exist) data = snapshot.val();
          console.log(data);
          that.setState({
